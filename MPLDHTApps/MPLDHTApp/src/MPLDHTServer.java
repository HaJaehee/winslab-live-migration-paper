...
/*
 * Update 2020/03/04
 *             Create: 2020-12-11 (1.0.0)
 *
 */
public final class MPLDHTServer {
    ...
    public static int ksvrPort = 18468;
    ...
    public static void main(String[] args) throws Exception {
	    ...
	    if (input == null) {
			if ((args.length == 1) ||
				...
				if(MPLDHTServer.logging){
				    System.out.println("Ambiguous Input.");
                    System.out.println("Usage: java MPLDHTServer [switchNum]");
                    System.out.println("Usage: java MPLDHTServer [switchNum] [bootstrap ip] [bootstrap port]");
                    System.out.println("Usage: java MPLDHTServer [switchNum] reset");
                    System.out.println("Usage: java MPLDHTServer [switchNum] logging");
                    System.out.println("Usage: java MPLDHTServer [switchNum] [bootstrap ip] [bootstrap port] logging");
                    System.out.println("Usage: java MPLDHTServer [switchNum] reset logging");
                }
				return;
			}
	    	...
	    	if (input.length == 1) {
				if(MPLDHTServer.logging)System.out.println("The First node begins.");
				kserver = new DHTServer(Integer.parseInt(input[0])-1);
				if(MPLDHTServer.logging)System.out.println("Bootstrap is done.");
			}
			else if (input.length == 3) {
				if(MPLDHTServer.logging)System.out.println("Connect to master node.");
        ...
        // MPLDHTServerHandler extends SimpleChannelInboundHandler which is used for an UDP socket server.
        // MPLDHTServerHandler communicates with its LMDHTServer and UDP Clients (e.g., viewers).
        PORT = 20001 + nodeIndex;
        ...
        try{
        	b.group(group)
        		.channel(NioDatagramChannel.class)
        		.handler(new MPLDHTServerHandler(nodeIndex, justReset, logging));
...
class MPLDHTServerHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    public MPLDHTServerHandler(int nodeIndex, boolean justReset, boolean logging) throws InterruptedException{
        super();


        System.out.println("Port "+MPLDHTServer.PORT+" is opened.");
    }
    ...
    public void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet) throws UnknownHostException, InterruptedException, Exception {
    	//if(MPLDHTServer.logging)System.err.println(packet.content().toString(CharsetUtil.UTF_8));
        ByteBuf payload = packet.content();

        //if(MPLDHTServer.logging)System.out.println("[Node "+MPLDHTServer.nodeNum+"] Received Message: "+payload.toString());
        ...
        if(MPLDHTServer.logging)System.out.printf("[Node %d] Received Message: opCode=%d,  switchNum=%d, IP=%s, SWIP = %s\n",MPLDHTServer.nodeIndex, opCode, switchNum, strIP, "127.0.0.1");
        
        if (opCode == OPCODE_BOOTUP){
        	if(MPLDHTServer.logging)System.out.println("opCode 0: show edge switchs list.");
            if(MPLDHTServer.logging)System.out.println("opCode 0 will be supported soon.");
            
        } /*else if (opCode == OPCODE_GET_HASH){  //deprecated by jaehee 170414
        	if(MPLDHTServer.logging)System.out.println("opCode 1: get Object ID from DHT server with digested IP");
        	//make Object ID and query to DHT table
			MessageDigest mDigest = MessageDigest.getInstance("SHA-256");
			byte[] strDig = mDigest.digest(strIP.getBytes());
	
			if(MPLDHTServer.logging)System.out.println("Hashed IP: "+strDig+", length: "+strDig.length);
			MPLDHTServer.kserver.get(opCode, strIP, switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+hostIP;
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 2: get Host IP from DHT server with Object ID.");
			...
			MPLDHTServer.kserver.get(opCode, sb.toString(), switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+esIP+hash;
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 3: store DHT server.");
            
            //byte[] byte_switch_ip = new byte[4];
            String strSWIP = MPLDHTServer.swIPAddrList[switchIndex];
            ...
            if(MPLDHTServer.logging)System.out.printf("[Node %d] Storing the pair: (Host IP=%s, Switch IP=%s)\n", MPLDHTServer.nodeIndex,strIP,strSWIP);
          
            MPLDHTServer.kserver.store(strIP, byteHostIP, byteSwitchIP);
			...
			for (int i = 0;i < MPLDHTServer.swCount;i++){
				if (MPLDHTServer.edgeSWList[i] != 0 && MPLDHTServer.edgeSWList[i] != switchNum){

                    byte[] swByte = ByteBuffer.allocate(2).putShort(MPLDHTServer.edgeSWList[i]).array();
				    sendBuf[1] = swByte[1];
				    int swListIndex = MPLDHTServer.edgeSWList[i]-1;
				    

			        Channel clientCh = MPLDHTServer.bClient.bind(0).sync().channel();
                    clientCh.writeAndFlush(
    	                        new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress(MPLDHTServer.swIPAddrList[swListIndex],MPLDHTServer.ovsPort))).addListener(ChannelFutureListener.CLOSE);
                }
			}
			byte[] swByte = ByteBuffer.allocate(2).putShort(switchNum).array();
			sendBuf[1] = swByte[1];
			if(MPLDHTServer.logging)System.out.printf("receiving SW IP="+"127.0.0.1"+",port="+MPLDHTServer.ovsPort+".\n");

	        Channel clientCh = MPLDHTServer.bClient.bind(0).sync().channel();
			
			clientCh.writeAndFlush(
    	         new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress("127.0.0.1",MPLDHTServer.ovsPort))).addListener(ChannelFutureListener.CLOSE);
...
//			String strInput = opCode+swNum+hostIP+swIP;
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 4: application mobility.");
            ...
            if(MPLDHTServer.logging){
	            ...
	            System.out.printf("[Node %d] Storing the pair: (Original Host IP=%s, Port Number=%s, New Host IP=%s, New Edge Switch IP=%s)\n", MPLDHTServer.nodeIndex, strHomeTargetHostIP, strPortNumber, strVisitingTargetHostIP, strVisitingESIP);
            }
            
            MPLDHTServer.kserver.store(strHomeTargetHostIP+strPortNumber, strVisitingTargetHostIP+strPortNumber, byteVisitingTargetHostIP, byteVisitingESIP, byteHomeTargetHostIP);
			...
			for (int i = 0;i < MPLDHTServer.swCount;i++){
                if (MPLDHTServer.edgeSWList[i] != 0 && MPLDHTServer.edgeSWList[i] != switchNum){
				    ...
				    sendBuf[1] = swByte[1];
				    int swListIndex = MPLDHTServer.edgeSWList[i]-1;
					if(MPLDHTServer.logging)System.out.printf("receiving SW IP="+MPLDHTServer.swIPAddrList[swListIndex]+",port="+MPLDHTServer.ovsPort+".\n");
...
//			String strInput = opCode+swNum+homeTargetHostIP+portNumber+esIP+newHostIP;
        ...
        //}
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 5: container mobility.");
            ...
            if(MPLDHTServer.logging){
	            ...
	            System.out.printf("[Node %d] Storing the pair: (Original Cnt IP=%s, New Ctn IP=%s, New Host IP=%s, New Edge Switch IP=%s)\n", MPLDHTServer.nodeIndex, strHomeCTIP, strVisitingCTIP, strVisitingTargetHostIP, strVisitingESIP);
            }
            MPLDHTServer.kserver.store(strHomeCTIP, strVisitingCTIP, byteVisitingCTIP, byteVisitingESIP, byteVisitingTargetHostIP, byteHomeCTIP);
...
//			String strInput = opCode+swNum+homeCtnIP+newCtnIP+newESIP+newHostIP; 
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 1: get Object ID from DHT server with digested IP");
        	//if(MPLDHTServer.logging)System.out.println("opCode 6: get ip:port");
            ...
            if(MPLDHTServer.logging)System.out.printf("[Node %d] Getting the pair: (Host IP:Port Number=%s%s)\n", MPLDHTServer.nodeIndex, strIP, strPortNumber);
            
            MPLDHTServer.kserver.get(OPCODE_GET_HASH, strIP+strPortNumber, switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+homeTargetHostIP+portNumber; 
        	...
        	if(MPLDHTServer.logging)System.out.println("opCode 101: Toggle ovs logging");
...
class ClientHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, DatagramPacket msg) {
        if(MPLDHTServer.logging)System.out.println("Must not be executed.");
    }
}
...
class DHTServer {
	...
	public DHTServer(int peerId) throws Exception {
		...
		peer = new PeerMaker(new Number160(rnd)).setPorts(MPLDHTServer.ksvrPort).setBindings(b).makeAndListen();
		if(MPLDHTServer.logging)System.out.println("My Peer ID = " + peer.getPeerID());
	}
	...
	public DHTServer(int peerId, String mIP, int port) throws Exception {
		...
		Bindings b = new Bindings();
		peer = new PeerMaker(new Number160(rnd)).setPorts(MPLDHTServer.ksvrPort + peerId).setBindings(b).makeAndListen();
		...
		if(opCode == OPCODE_GET_HASH){
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						...
						if(MPLDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_HASH+", " + future.getData().getObject().toString());
				        ...
				        Channel clientCh = MPLDHTServer.bClient.bind(0).sync().channel();
						clientCh.writeAndFlush(
							new DatagramPacket(Unpooled.copiedBuffer(sendData), new InetSocketAddress("localhost",MPLDHTServer.ovsPort))).addListener(ChannelFutureListener.CLOSE);
						...
						if(MPLDHTServer.logging)System.out.println("Get Failed.");
						
						byte[] firstSHA = sha256(input).getBytes();
						MPLDHTServer.kserver.get(OPCODE_GET_IPPORT, input, switchNum, lbyteHostIP, firstSHA);
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						...
						if(MPLDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_IP+", " + future.getData().getObject().toString());
						String foundData = future.getData().getObject().toString();
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun implements sending UDP packet to OVS
						if(MPLDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_IPPORT+", " + future.getData().getObject().toString());
						String foundData = future.getData().getObject().toString();
			...
			if(MPLDHTServer.logging)System.out.println("Logical Error");
		}
	}
	...
	public void store(String strHostIP, byte[] hostIP, byte[] switchIP) throws IOException, NoSuchAlgorithmException {
		...
		if(MPLDHTServer.logging)System.out.println("key: "+Number160.createHash(firstSHA)+", data: "+jsonString.toString());
	...
	public void store(String originalHostIPPort, String visitingTargetHostIPPort, byte[] visitingTargetHostIP, byte[] switchIP, byte[] homeTargetHostIP) throws IOException, NoSuchAlgorithmException {
		...
		if(MPLDHTServer.logging)System.out.println("key: "+Number160.createHash(firstSHA_2)+", data: "+jsonString.toString());
	...
	public void get(final int opCode, final String input, final byte switchNum, final byte[] byte_host_ip, final byte[] hashedIP) throws ClassNotFoundException, IOException, NoSuchAlgorithmException {
		...
		if(opCode == MPLDHTServerHandler.OPCODE_GET_HASH){
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun implements sending UDP packet to OVS
						if(MPLDHTServer.logging)System.out.println("OpCode = "+MPLDHTServerHandler.OPCODE_GET_HASH+", " + future.getData().getObject().toString());
						...
						MPLDHTServer.client_ch.writeAndFlush(
							new DatagramPacket(Unpooled.copiedBuffer(send_data), new InetSocketAddress("localhost",MPLDHTServer.ovsPort))).addListener(ChannelFutureListener.CLOSE);
		...
		} else if(opCode == MPLDHTServerHandler.OPCODE_GET_IP){
			//In this case, input is an objectKey
			FutureDHT futureDHT = peer.get(Number160.createHash(input)).start();
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				@Override
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun needs to implement sending UDP packet to OVS
						if(MPLDHTServer.logging)System.out.println("OpCode = "+MPLDHTServerHandler.OPCODE_GET_IP+", " + future.getData().getObject().toString());
						String recv_data = future.getData().getObject().toString();
						byte[] send_data = new byte[42];//Jaehee modified 160720
