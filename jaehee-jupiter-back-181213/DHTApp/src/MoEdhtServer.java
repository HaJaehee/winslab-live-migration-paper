...
public final class MoEDHTServer {
    ...
    public static void main(String[] args) throws Exception {
	    ...
	    if (input == null) {
			if (args.length == 1) {
				if(MoEDHTServer.logging)System.out.println("The First node begins");
				kserver = new DHTServer(Integer.parseInt(args[0]));
				if(MoEDHTServer.logging)System.out.println("Bootstrap is done");
			}
			else if (args.length == 3) {
				if(MoEDHTServer.logging)System.out.println("Connect to master node");
				kserver = new DHTServer(Integer.parseInt(args[0]),args[1],Integer.parseInt(args[2]));
				if(MoEDHTServer.logging)System.out.println("Bootstrap is done");
			}
			else{
				if(MoEDHTServer.logging)System.out.println("Ambiguous Input. Usage: java MoEdhtServer [nodeNum] {[bootstrap ip] [boostrap port]}");
				return;
			}
        ...
        try{
        	b.group(group)
        		.channel(NioDatagramChannel.class)
        		.handler(new MoEDHTServerHandler(nodeIndex));
...
class MoEDHTServerHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    private static final int MOE_HDR_LENGTH = 32;
    
    public MoEDHTServerHandler(int nodeIndex) throws InterruptedException{
        super();
        
        int sendBufLength = 5 + 2*MoEDHTServer.swCount;
    	byte[] sendBuf = new byte[sendBufLength];
        sendBuf[0] = OPCODE_SET_SWTYPE;
        sendBuf[1] = (byte)MoEDHTServer.edgeSWList[nodeIndex];//switch num
        sendBuf[2] = (byte)0x02;//switch type

        byte[] lengthBytes = ByteBuffer.allocate(2).putShort((short)MoEDHTServer.swCount).array();
        sendBuf[3] = lengthBytes[0];
        sendBuf[4] = lengthBytes[1];
        
        //appends edge switch numbers to byte array 
        for (int i = 0; i < MoEDHTServer.swCount; i++){
            if (MoEDHTServer.edgeSWList[i] != 0){
                byte[] swBytes = ByteBuffer.allocate(2).putShort((short)(MoEDHTServer.edgeSWList[i]-1)).array();
                sendBuf[5 + i*2] = swBytes[0];
                sendBuf[5 + i*2+1] = swBytes[1];
            }
        }
        if(MoEDHTServer.logging) {
        ...
        MoEDHTServer.clientCh.writeAndFlush(
            new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress("localhost",MoEDHTServer.ovsPort))).sync();
    ...
    public void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet) throws UnknownHostException, InterruptedException, Exception {
    	//if(MoEDHTServer.logging)System.err.println(packet.content().toString(CharsetUtil.UTF_8));
        ByteBuf payload = packet.content();
        
        //if(MoEDHTServer.logging)System.out.println("[Node "+MoEDHTServer.nodeNum+"] Received Message: "+payload.toString());
        ...
        if(MoEDHTServer.logging)System.out.printf("[Node %d] Received Message: opCode=%d,  switchNum=%d, IP=%s, SWIP = %s\n",MoEDHTServer.nodeIndex, opCode, switchNum, strIP, "127.0.0.1");
        
        if (opCode == OPCODE_BOOTUP){
        	if(MoEDHTServer.logging)System.out.println("opCode 0: show edge switchs list");
            if(MoEDHTServer.logging)System.out.println("opCode 0 will be supported soon");
            
        } /*else if (opCode == OPCODE_GET_HASH){  //deprecated by jaehee 170414
        	if(MoEDHTServer.logging)System.out.println("opCode 1: get Object ID from DHT server with digested IP");
        	//make Object ID and query to DHT table
			MessageDigest mDigest = MessageDigest.getInstance("SHA-256");
			byte[] strDig = mDigest.digest(strIP.getBytes());
	
			if(MoEDHTServer.logging)System.out.println("Hashed IP: "+strDig+", length: "+strDig.length);
			MoEDHTServer.kserver.get(opCode, strIP, switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+hostIP;
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 2: get Host IP from DHT server with Object ID");
        	//copy payload(Object ID) to strDig byte array and query to DHT table
			byte[] strDig = new byte[MOE_HDR_LENGTH]; //Jaehee modified 160720
			
			for (int i = 0;i < MOE_HDR_LENGTH;i++){ //Jaehee modified 160720
			...
			if(MoEDHTServer.logging){
				...
				for (int i = 0;i < MOE_HDR_LENGTH;i++){ 
			...
			MoEDHTServer.kserver.get(opCode, sb.toString(), switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+esIP+hash;
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 3: store DHT server");
            
            //byte[] byte_switch_ip = new byte[4];
            String strSWIP = MoEDHTServer.swIPAddrList[switchIndex];
            ...
            if(MoEDHTServer.logging)System.out.printf("[Node %d] Storing the pair: (Host IP=%s, Switch IP=%s)\n", MoEDHTServer.nodeIndex,strIP,strSWIP);
          
            MoEDHTServer.kserver.store(strIP, byteHostIP, byteSwitchIP);
			...
			for (int i = 0; i < MOE_HDR_LENGTH;i++){//Jaehee modified 170329
			...
			for (int i = 0;i < MoEDHTServer.swCount;i++){
				if (MoEDHTServer.edgeSWList[i] != 0 && MoEDHTServer.edgeSWList[i] != switchNum){

                    byte[] swByte = ByteBuffer.allocate(2).putShort(MoEDHTServer.edgeSWList[i]).array();
				    sendBuf[1] = swByte[1];
				    int swListIndex = MoEDHTServer.edgeSWList[i]-1;
                    MoEDHTServer.clientCh.writeAndFlush(
    	                        new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress(MoEDHTServer.swIPAddrList[swListIndex],MoEDHTServer.ovsPort))).sync();
                }
			}
			byte[] swByte = ByteBuffer.allocate(2).putShort(switchNum).array();
			sendBuf[1] = swByte[1];
			System.out.printf("receiving SW IP="+"127.0.0.1"+",port="+MoEDHTServer.ovsPort+"\n");
			MoEDHTServer.clientCh.writeAndFlush(
    	         new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress("127.0.0.1",MoEDHTServer.ovsPort))).sync();
...
//			String strInput = opCode+swNum+hostIP+swIP;
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 4: application mobility");
            ...
            if(MoEDHTServer.logging){
	            ...
	            System.out.printf("[Node %d] Storing the pair: (Original Host IP=%s, Port Number=%s, New Host IP=%s, New Edge Switch IP=%s)\n", MoEDHTServer.nodeIndex, strHomeTargetHostIP, strPortNumber, strVisitingTargetHostIP, strVisitingESIP);
            }
            
            MoEDHTServer.kserver.store(strHomeTargetHostIP+strPortNumber, strVisitingTargetHostIP+strPortNumber, byteVisitingTargetHostIP, byteVisitingESIP, byteHomeTargetHostIP);
			...
			for (int i = 0;i < MoEDHTServer.swCount;i++){
                if (MoEDHTServer.edgeSWList[i] != 0 && MoEDHTServer.edgeSWList[i] != switchNum){
				    ...
				    sendBuf[1] = swByte[1];
				    int swListIndex = MoEDHTServer.edgeSWList[i]-1;
					System.out.printf("receiving SW IP="+MoEDHTServer.swIPAddrList[swListIndex]+",port="+MoEDHTServer.ovsPort+"\n");
                    MoEDHTServer.clientCh.writeAndFlush(
    	                        new DatagramPacket(Unpooled.copiedBuffer(sendBuf), new InetSocketAddress(MoEDHTServer.swIPAddrList[swListIndex],MoEDHTServer.ovsPort))).sync();
                }
			}
...
//			String strInput = opCode+swNum+homeTargetHostIP+portNumber+esIP+newHostIP;
        ...
        //}
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 5: container mobility");
            ...
            if(MoEDHTServer.logging){
	            ...
	            System.out.printf("[Node %d] Storing the pair: (Original Cnt IP=%s, New Ctn IP=%s, New Host IP=%s, New Edge Switch IP=%s)\n", MoEDHTServer.nodeIndex, strHomeCTIP, strVisitingCTIP, strVisitingTargetHostIP, strVisitingESIP);
            }
            MoEDHTServer.kserver.store(strHomeCTIP, strVisitingCTIP, byteVisitingCTIP, byteVisitingESIP, byteVisitingTargetHostIP, byteHomeCTIP);
...
//			String strInput = opCode+swNum+homeCtnIP+newCtnIP+newESIP+newHostIP; 
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 1: get Object ID from DHT server with digested IP");
        	//if(MoEDHTServer.logging)System.out.println("opCode 6: get ip:port");
            ...
            if(MoEDHTServer.logging)System.out.printf("[Node %d] Getting the pair: (Host IP:Port Number=%s%s)\n", MoEDHTServer.nodeIndex, strIP, strPortNumber);
            
            MoEDHTServer.kserver.get(OPCODE_GET_HASH, strIP+strPortNumber, switchNum, byteHostIP, strDig);
...
//			String strInput = opCode+swNum+homeTargetHostIP+portNumber; 
        	...
        	if(MoEDHTServer.logging)System.out.println("opCode 101: Toggle ovs logging");
...
class ClientHandler extends SimpleChannelInboundHandler<DatagramPacket> {
    ...
    public void channelRead0(ChannelHandlerContext ctx, DatagramPacket msg) {
        if(MoEDHTServer.logging)System.out.println("Must not be executed");
    }
}
...
class DHTServer {
	...
	public DHTServer(int peerId) throws Exception {
		...
		peer = new PeerMaker(new Number160(rnd)).setPorts(MoEDHTServer.ksvrPort).setBindings(b).makeAndListen();
		if(MoEDHTServer.logging)System.out.println("My Peer ID = " + peer.getPeerID());
	}
	...
	public DHTServer(int peerId, String mIP, int port) throws Exception {
		...
		Bindings b = new Bindings();
		peer = new PeerMaker(new Number160(rnd)).setPorts(MoEDHTServer.ksvrPort + peerId).setBindings(b).makeAndListen();
		...
		if(opCode == OPCODE_GET_HASH){
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						...
						if(MoEDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_HASH+", " + future.getData().getObject().toString());
						...
						for (int i = 0; i < 4;i++){
							...
							sendData[6+MOE_HDR_LENGTH+(3-i)] = (byte) ((Character.digit(recvData.charAt((i+4)*2), 16) << 4) + Character.digit(recvData.charAt((i+4)*2+1), 16));
						...
						MoEDHTServer.clientCh.writeAndFlush(
							new DatagramPacket(Unpooled.copiedBuffer(sendData), new InetSocketAddress("localhost",MoEDHTServer.ovsPort))).sync();
						...
						if(MoEDHTServer.logging)System.out.println("Get Failed");
						
						byte[] firstSHA = sha256(input).getBytes();
						MoEDHTServer.kserver.get(OPCODE_GET_IPPORT, input, switchNum, lbyteHostIP, firstSHA);
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						...
						if(MoEDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_IP+", " + future.getData().getObject().toString());
						String foundData = future.getData().getObject().toString();
						...
						for (int i = 0; i < 4;i++){
							...
							sendData[6+MOE_HDR_LENGTH+i] = 0x00;
						}//Jaehee modified 160720
						for (int i = 0; i < MOE_HDR_LENGTH;i++){ //Jaehee modified 160720
							sendData[6+i]=  hashedIP[i];
						}
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun implements sending UDP packet to OVS
						if(MoEDHTServer.logging)System.out.println("OpCode = "+OPCODE_GET_IPPORT+", " + future.getData().getObject().toString());
						String foundData = future.getData().getObject().toString();
			...
			if(MoEDHTServer.logging)System.out.println("Logical Error");
		}
	}
	...
	public void store(String strHostIP, byte[] hostIP, byte[] switchIP) throws IOException, NoSuchAlgorithmException {
		...
		if(MoEDHTServer.logging)System.out.println("key: "+Number160.createHash(firstSHA)+", data: "+jobj.toString());
	...
	public void store(String originalHostIPPort, String visitingTargetHostIPPort, byte[] visitingTargetHostIP, byte[] switchIP, byte[] homeTargetHostIP) throws IOException, NoSuchAlgorithmException {
		...
		if(MoEDHTServer.logging)System.out.println("key: "+Number160.createHash(firstSHA_2)+", data: "+jobj2.toString());
	...
	public void get(final int opCode, final String input, final byte switchNum, final byte[] byte_host_ip, final byte[] hashedIP) throws ClassNotFoundException, IOException, NoSuchAlgorithmException {
		...
		if(opCode == MoEDHTServerHandler.OPCODE_GET_HASH){
			...
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				...
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun implements sending UDP packet to OVS
						if(MoEDHTServer.logging)System.out.println("OpCode = "+MoEDHTServerHandler.OPCODE_GET_HASH+", " + future.getData().getObject().toString());
						...
						MoEDHTServer.client_ch.writeAndFlush(
							new DatagramPacket(Unpooled.copiedBuffer(send_data), new InetSocketAddress("localhost",MoEDHTServer.ovsPort))).sync();
		...
		} else if(opCode == MoEDHTServerHandler.OPCODE_GET_IP){
			//In this case, input is an objectKey
			FutureDHT futureDHT = peer.get(Number160.createHash(input)).start();
			futureDHT.addListener(new BaseFutureAdapter<FutureDHT>() {
				@Override
				public void operationComplete(FutureDHT future)
					...
					if (future.isSuccess()) {
						//Jaehyun needs to implement sending UDP packet to OVS
						if(MoEDHTServer.logging)System.out.println("OpCode = "+MoEDHTServerHandler.OPCODE_GET_IP+", " + future.getData().getObject().toString());
						String recv_data = future.getData().getObject().toString();
						byte[] send_data = new byte[42];//Jaehee modified 160720
