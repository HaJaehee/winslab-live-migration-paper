...
//jaehee & jaehyun modified  2018/02/19
static int32_t moe_AddHeader(struct sk_buff *skb, uint32_t newSrcIP, uint8_t* hashed, uint32_t esIP, int proto, int doInsertObjID)
{
	uint8_t* data = NULL;
...
static int32_t moe_CheckHeader(struct vport *vp, struct sk_buff *skb)
{
	...
	if (protocol == ETH_P_ARP && senderType == SENDERTYPE_UE) {
		...
		if ((tp_protocol == IPPROTO_ICMP) ){
			...
			if ((IHL == 0x45 && (totalLen - IP_HLEN >= 18) && ((dstIP&0x00ffffff)!=(SWITCHS_IP[switchNum-1]&0x00ffffff)) && dstIP != 16777343 && dstIP != SWITCHS_IP[switchNum-1]) || (senderType == SENDERTYPE_SW && IHL == 0x45 && !dstIPisSWIP)) {
					//normal IP packet
					//dstIP is not 127.0.0.1 and not this switch's ip 
					//host is in other subnet
					//or 
					//host is in its subnet
				//data += IP_HLEN;
