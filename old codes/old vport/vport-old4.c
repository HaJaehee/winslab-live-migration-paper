...
int ovs_vport_init(void)
{
	...
	os_WriteLog("--- OvS with MoE has successfully been loaded. --- \n");
...
//jaehee & jaehyun modified  2018-02-14
static int32_t moe_AddHeader(struct sk_buff *skb, uint32_t newSrcIP, uint8_t* hashed, uint32_t esIP, int proto, int doInsertObjID)
{
	uint8_t* data = NULL;
...
static int32_t moe_CheckHeader(struct vport *vp, struct sk_buff *skb)
{
	...
	if (protocol == ETH_P_ARP && senderType == SENDERTYPE_UE) {
		...
		if ((tp_protocol == IPPROTO_ICMP) || (senderType == SENDERTYPE_SW && IHL == 0x45 && !dstIPisSWIP)){
			if(LOGGING){
			...
			if (IHL == 0x45 && (totalLen - IP_HLEN >= 18) && ((dstIP&0x00ffffff)!=(SWITCHS_IP[switchNum-1]&0x00ffffff)) && dstIP != 16777343 && dstIP != SWITCHS_IP[switchNum-1]) {
				...
				*((uint8_t*)&newIP + 0), *((uint8_t*)&newIP + 1), *((uint8_t*)&newIP + 2), *((uint8_t*)&newIP + 3));}
				...
				if (newIP == 0) {
					return moe_AddHeader(skb, srcIP, hashed, 0, IPPROTO_ICMP, 0);
				}
				
				return moe_AddHeader(skb, srcIP, hashed, newIP, IPPROTO_ICMP, 1);
		...
		if ((senderType == SENDERTYPE_UE && IHL == 0x45) || (senderType == SENDERTYPE_SW && IHL == 0x45 && !dstIPisSWIP))  {
			...
			*((uint8_t*)&newIP + 0), *((uint8_t*)&newIP + 1), *((uint8_t*)&newIP + 2), *((uint8_t*)&newIP + 3));}
			...
			if (newIP == 0) {
				return moe_AddHeader(skb, srcIP, hashed, 0, tp_protocol, 0);
			}
			
			return moe_AddHeader(skb, srcIP, hashed, newIP, tp_protocol, 1);
		}
