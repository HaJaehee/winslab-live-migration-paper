...
//Update history: MoE(2016) > SOMO(2017) > LM(2018)
...
Rev. history : 2018-03-16
Version : 1.0.0
	Replaced from notations of moe to notations of lm (ex: oldIP --> homeIP, newIP --> visitingIP).
Modifier : Jaehee Ha (jaehee.ha@kaist.ac.kr)

Rev. history : 2018-03-18
Version : 1.0.1
	Revised some strings in the print function.
Modifier : Jaehee Ha (jaehee.ha@kaist.ac.kr)
*/
/* -------------------------------------------------------- */
...
typedef struct object_entry {
	...
	struct hlist_node hlist_visiting_ip;
...
static void lm_CleanUp(void);
static void lm_InsertObject(uint32_t switchNum, uint32_t destIP, uint16_t destPort, uint8_t* objHash, uint32_t switchIP, uint32_t visitingIP);
static uint32_t lm_GetHomeIPFromSrcIP(uint32_t switchNum, uint32_t srcIP, uint16_t srcPort, uint8_t** objHash, uint32_t* homeIP);
static uint32_t lm_GetObjectFromIP(uint32_t switchNum, uint32_t destIP, uint16_t destPort, uint8_t** objHash, uint32_t* pSwitchIP);
static uint32_t lm_GetObjectFromHash(uint32_t switchNum, uint8_t* objHash, uint32_t* pHomeIP, uint32_t* pVisitingIP);
static void lm_DeleteObjectAll(uint32_t switchNum, uint32_t destIP, uint16_t destPort);
static void lm_DeleteObjectHash(uint32_t switchNum, uint8_t* objHash);
static void lm_DeleteMOIPAll(uint32_t switchNum, uint32_t visitingIP, uint16_t destPort);
static void lm_SaveSKB(uint32_t switchNum, uint32_t destIP, struct vport *vp, struct sk_buff *skb); //SKB = Socket Buffer
static void lm_ForwardSKB(uint32_t switchNum, uint32_t destIP);
static int32_t lm_CheckHeader(struct vport *vp, struct sk_buff *skb);
static int32_t lm_AddHeader(struct sk_buff *skb, uint32_t visitingSrcIP, uint8_t* hashed, uint32_t esIP, int proto, int doInsertObjID);
static int32_t lm_RemoveHeader(struct sk_buff *skb, uint32_t homeIP, uint32_t visitingIP, int proto);
static int32_t lm_ChangeHeader(struct sk_buff *skb, uint32_t homeDstIP, uint32_t visitingDstIP, uint32_t homeSrcIP, uint32_t visitingSrcIP, int proto);
...
static void ipc_ReceiveMessages(struct work_struct* data)
{
	...
	while ((len = skb_queue_len(&wrapper->sk->sk_receive_queue)) > 0) {
		...
		if(LOGGING){os_WriteLog2("Received a UDP message, op Code=%u, Switch Num=%u\n", opCode, switchNum);}
		...
		if (opCode == OPCODE_SET_SWTYPE) {
			...
			if(LOGGING){os_WriteLog3("Switch Num=%u, Execution Time=%u.%06d\n", i, STAT_TIMES[i].tv_sec, STAT_TIMES[i].tv_usec);} }} }
			*/
			...
			if (!lm_GetObjectFromIP(switchNum, destIP, 0, &tempHash, &temp)) {		// If not exist,
				...
				*((uint8_t*)&switchIP + 0), *((uint8_t*)&switchIP + 1), *((uint8_t*)&switchIP + 2), *((uint8_t*)&switchIP + 3));}
				lm_InsertObject(switchNum, destIP, 0, objHash, switchIP, 0);
				lm_ForwardSKB(switchNum, destIP);
			}
			...
			if (!lm_GetObjectFromIP(switchNum, destIP, 0, &tempHash, &temp)) {		// If not exist,
				...
				lm_InsertObject(switchNum, destIP, 0, objHash, switchIP, 0);
				lm_ForwardSKB(switchNum, switchIP); // Should be the switch's IP
			}
			...
			if (lm_GetObjectFromIP(switchNum, destIP, 0, &tempHash, &temp) && switchIP != temp) {
			...
			destIP = *(uint32_t*)(skb->data + UDP_HLEN + 2);
			switchIP = *(uint32_t*)(skb->data + UDP_HLEN + 6);
			visitingIP = *(uint32_t*)(skb->data + UDP_HLEN + 10);
			destPort = ntohs(*(uint16_t*)(skb->data + UDP_HLEN + 14));
			//if (lm_GetObjectFromIP(switchNum, destIP, 0, &tempHash, &temp) && switchIP != temp) {
				if(LOGGING){os_WriteLog9("Application Mobility Support! Switch IP=%u.%u.%u.%u when Host IP=%u.%u.%u.%u with dst Port=%u\n",
					*((uint8_t*)&switchIP + 0), *((uint8_t*)&switchIP + 1), *((uint8_t*)&switchIP + 2), *((uint8_t*)&switchIP + 3),
				*((uint8_t*)&destIP + 0), *((uint8_t*)&destIP + 1), *((uint8_t*)&destIP + 2), *((uint8_t*)&destIP + 3), destPort);}
				memcpy(objHash, skb->data + UDP_HLEN + 16, HASH_LEN);
				lm_InsertObject(switchNum, destIP, destPort, objHash, switchIP, visitingIP);
			//}
		} /*else if (opCode == OPCODE_NEW_CTN) { // Jaehee: Is it necessarily needed?
			uint8_t* tempHash;
			uint32_t visitingIP, temp = 0;
            uint16_t destPort = 0;
			destIP = *(uint32_t*)(skb->data + UDP_HLEN + 2);
			switchIP = *(uint32_t*)(skb->data + UDP_HLEN + 6);
			visitingIP = *(uint32_t*)(skb->data + UDP_HLEN + 10);
			if (lm_GetObjectFromIP(switchNum, destIP, 0, &tempHash, &temp) && switchIP != temp) {
				...
				*((uint8_t*)&destIP + 0), *((uint8_t*)&destIP + 1), *((uint8_t*)&destIP + 2), *((uint8_t*)&destIP + 3));}
				...
				lm_InsertObject(switchNum, destIP, 0, objHash, switchIP, visitingIP);
			}
		}*/ 
	...
	uint32_t clientIP, uint8_t* data)
{
	...
	if(LOGGING){os_WriteLog2("Sending a message to the upper layer. Switch Num=%u, op Code=%u\n", switchNum, opCode);}
	sendBuffer[0] = opCode;
...
{
	{
		...
		hash_for_each(OBJ_TBL, bucket, current_entry, hlist_ip) {
			if (previous_entry != NULL) {
				...
				if (previous_entry->hlist_hash.next == NULL && previous_entry->hlist_hash.pprev == NULL && previous_entry->hlist_visiting_ip.next == NULL && previous_entry->hlist_visiting_ip.pprev == NULL)
		...
		hash_for_each(OBJ_REV_TBL, bucket, current_entry, hlist_hash) {
			if (previous_entry != NULL) {
				...
				if (previous_entry->hlist_ip.next == NULL && previous_entry->hlist_ip.pprev == NULL && previous_entry->hlist_visiting_ip.next == NULL && previous_entry->hlist_visiting_ip.pprev == NULL)
					kfree(previous_entry);
			}
			previous_entry = current_entry;
		}
		...
		previous_entry = NULL;
		hash_for_each(OBJ_MOIP_TBL, bucket, current_entry, hlist_visiting_ip) {
			if (previous_entry != NULL) {
				hash_del(&previous_entry->hlist_visiting_ip);
...
static void lm_InsertObject(uint32_t switchNum, uint32_t destIP, uint16_t destPort, uint8_t* objHash, uint32_t switchIP, uint32_t visitingIP)
{
	...
	_OE* entry = NULL;
	if(LOGGING){
		os_WriteLog10("Updating cache: Switch Num=%u, Dest IP=%u.%u.%u.%u, Dest Port=%u, Switch IP=%u.%u.%u.%u,\n",switchNum, *((uint8_t*)&destIP + 0), *((uint8_t*)&destIP + 1), *((uint8_t*)&destIP + 2), *((uint8_t*)&destIP + 3), destPort, *((uint8_t*)&switchIP + 0), *((uint8_t*)&switchIP + 1), *((uint8_t*)&switchIP + 2), *((uint8_t*)&switchIP + 3));
		os_WriteLog4("Visiting IP=%u.%u.%u.%u\n", *((uint8_t*)&visitingIP + 0), *((uint8_t*)&visitingIP + 1), *((uint8_t*)&visitingIP + 2), *((uint8_t*)&visitingIP + 3));
	}
	entry = kmalloc(sizeof(_OE), GFP_KERNEL);
	lm_DeleteObjectAll(switchNum, destIP, destPort);
	entry->switchNum = switchNum;
	entry->destIP = destIP;
	entry->destPort = destPort;
	entry->switchIP = switchIP;
	entry->visitingIP = visitingIP;
	...
	INIT_HLIST_NODE(&entry->hlist_visiting_ip);
	hash_add(OBJ_MOIP_TBL, &entry->hlist_visiting_ip, entry->visitingIP);
		...
		hash_for_each_possible(OBJ_TBL, current_entry, hlist_ip, destIP) {
			if (current_entry->switchNum == switchNum &&
				...
				*pSwitchIP = current_entry->switchIP;
				if(LOGGING){
					os_WriteLog6("Cache hit! Switch Num=%u, Dest IP=%u.%u.%u.%u, Dest Port=%u\n", switchNum, *((uint8_t*)&destIP + 0), *((uint8_t*)&destIP + 1), *((uint8_t*)&destIP + 2), *((uint8_t*)&destIP + 3), destPort);
...
static uint32_t lm_GetHomeIPFromSrcIP(uint32_t switchNum, uint32_t srcIP, uint16_t srcPort, uint8_t** objHash, uint32_t* homeIP)
{
	...
	_OE* current_entry = NULL;
	uint32_t visitingIP = srcIP;
	
	if (srcPort != (uint16_t)0 && (srcPort < (uint16_t)32768 || srcPort > (uint16_t)61000)) {
		if(LOGGING){os_WriteLog6("Searching cache. Switch Num=%u, Src IP=%u.%u.%u.%u, Src Port=%u\n", switchNum, *((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3), srcPort);}
		
		hash_for_each_possible(OBJ_MOIP_TBL, current_entry, hlist_visiting_ip, visitingIP) {
			if (current_entry->switchNum == switchNum &&
				current_entry->visitingIP == visitingIP && current_entry->destPort == srcPort) {
				*homeIP = current_entry->destIP;
				if(LOGGING){os_WriteLog6("Cache hit! Switch Num=%u, Home IP=%u.%u.%u.%u, Src Port=%u\n", switchNum, *((uint8_t*)homeIP + 0), *((uint8_t*)homeIP + 1), *((uint8_t*)homeIP + 2), *((uint8_t*)homeIP + 3), srcPort);}
	...
	hash_for_each_possible(OBJ_MOIP_TBL, current_entry, hlist_visiting_ip, visitingIP) {
		if (current_entry->switchNum == switchNum &&
			current_entry->visitingIP == visitingIP && current_entry->destPort == (uint16_t)0) {
			*homeIP = current_entry->destIP;
			if(LOGGING){os_WriteLog6("Cache hit! Switch Num=%u, Home IP=%u.%u.%u.%u, Src Port=%u\n", switchNum, *((uint8_t*)homeIP + 0), *((uint8_t*)homeIP + 1), *((uint8_t*)homeIP + 2), *((uint8_t*)homeIP + 3), (uint16_t)0);}
...
static uint32_t lm_GetObjectFromHash(uint32_t switchNum, uint8_t* objHash, uint32_t* pHomeIP, uint32_t* pVisitingIP)
{
	...
	int i = 0;
	if(LOGGING){
		os_WriteLog1("Searching cache. Switch Num=%u\n", switchNum);
		os_WriteLog("ObjID=\n");
	...
	hash_for_each_possible(OBJ_REV_TBL, current_entry, hlist_hash, *(uint64_t*)objHash) {
		if (current_entry->switchNum == switchNum && memcmp(current_entry->objHash, objHash, HASH_LEN) == 0) {
			*pHomeIP = current_entry->destIP;
			if (current_entry->visitingIP == 0) {
				*pVisitingIP = current_entry->destIP;
			} 
			else {
				*pVisitingIP = current_entry->visitingIP;
			}
			if(LOGGING){
				os_WriteLog4("Cache hit! Home Dest IP=%u.%u.%u.%u\n", *((uint8_t*)pHomeIP + 0), *((uint8_t*)pHomeIP + 1), *((uint8_t*)pHomeIP + 2), *((uint8_t*)pHomeIP + 3));
				os_WriteLog5("Switch Num=%u, Visiting Dest IP=%u.%u.%u.%u\n", switchNum, *((uint8_t*)pVisitingIP + 0), *((uint8_t*)pVisitingIP + 1), *((uint8_t*)pVisitingIP + 2), *((uint8_t*)pVisitingIP + 3));
				}
			return 1;
		}
	}
...
static void lm_DeleteObjectAll(uint32_t switchNum, uint32_t destIP, uint16_t destPort)
{
	...
	hash_for_each_possible(OBJ_TBL, current_entry, hlist_ip, destIP) {
		if (current_entry->switchNum == switchNum &&
			...
			if (current_entry->hlist_hash.next == NULL && current_entry->hlist_hash.pprev == NULL && current_entry->hlist_visiting_ip.next == NULL && current_entry->hlist_visiting_ip.pprev == NULL)
				kfree(current_entry);
			else
				lm_DeleteObjectHash(switchNum, current_entry->objHash);
				lm_DeleteMOIPAll(switchNum, current_entry->visitingIP, destPort);
			return;
		}
	}
...
static void lm_DeleteObjectHash(uint32_t switchNum, uint8_t* objHash)
{
	...
	hash_for_each_possible(OBJ_REV_TBL, current_entry, hlist_hash, *(uint64_t*)objHash) {
		if (current_entry->switchNum == switchNum && memcmp(current_entry->objHash, objHash, HASH_LEN) == 0) {
			...
			if (current_entry->hlist_ip.next == NULL && current_entry->hlist_ip.pprev == NULL && current_entry->hlist_visiting_ip.next == NULL && current_entry->hlist_visiting_ip.pprev == NULL)
				kfree(current_entry);
			return;
		}
	}
...
static void lm_DeleteMOIPAll(uint32_t switchNum, uint32_t visitingIP, uint16_t destPort)
{
	...
	hash_for_each_possible(OBJ_MOIP_TBL, current_entry, hlist_visiting_ip, visitingIP) {
		if (current_entry->switchNum == switchNum && current_entry->visitingIP == visitingIP && current_entry->destPort == destPort) {
			hash_del(&current_entry->hlist_visiting_ip);
...
static void lm_SaveSKB(uint32_t switchNum, uint32_t destIP, struct vport *vp, struct sk_buff *skb)
{
	_LE* entry = NULL;
...
static void lm_ForwardSKB(uint32_t switchNum, uint32_t destIP)
{
	...
	list_for_each_entry(entry, &SKB_LIST, list) {
		...
		if (entry->switchNum == switchNum && entry->destIP == destIP) {
			if (lm_CheckHeader(entry->vp, entry->skb) == -1) {continue;}
...
// LOHan: MoE Support
// Jaehee: LM Support
static void lm_CheckNewUE(const uint8_t switchNum, const uint16_t protocol, uint8_t* data)
{
	uint32_t senderIP = 0;
	...
	if(LOGGING){
		if (++STAT_NEW_UES[switchNum] == 1000) {
			os_WriteLog1("Switch Num=%u, 1K new UEs are connected.\n", switchNum);
...
static int32_t lm_AddHeader(struct sk_buff *skb, uint32_t visitingSrcIP, uint8_t* hashed, uint32_t esIP, int proto, int doInsertObjID)
{
	...
	uint32_t homeSrcIP = 0;
	uint16_t pre16 = 0, post16 = 0;
	uint32_t sum = 0;

	if(doInsertObjID) {
		if(LOGGING){os_WriteLog("Adding header operation.\n");}
		if (skb_cow_head(skb, LM_HLEN) < 0){
			return -1;
		}

		skb_push(skb, LM_HLEN);
		/* Move the mac addresses to the beginning of the new header. */
		memmove(skb->data, skb->data + LM_HLEN, ETH_HLEN + IP_HLEN);
		...
		temp = htons(CalculateIPChecksum(data, IP_HLEN + LM_HLEN));
	...
	//tcph_len = (*(uint8_t*)(data + IP_HLEN + LM_HLEN + 12) & 0xf0) >> 4;
	//if(LOGGING){print_hex_dump(KERN_ALERT, "data ", DUMP_PREFIX_OFFSET, 16, 1, data + IP_HLEN + LM_HLEN + (tcph_len*4), ntohs(len) - IP_HLEN - LM_HLEN - (tcph_len*4), 1);}

	if (proto != IPPROTO_ICMP && visitingSrcIP != homeSrcIP) {
		
		if (proto == IPPROTO_UDP) {
			memcpy(&org_tp_check, data + IP_HLEN + LM_HLEN + 6, sizeof(uint16_t));
			if(LOGGING){os_WriteLog1("Original udp checksum=%04x\n",ntohs(org_tp_check));}
		} // UDP calculates checksum
		
		
//		if (proto == IPPROTO_UDP) {
//			memcpy(data + IP_HLEN + LM_HLEN + 6, (void*)"\x0000", sizeof(uint16_t));
//			
//		} //UDP don't calculates checksum
		
		else if (proto == IPPROTO_TCP) {
			memcpy(&org_tp_check, data + IP_HLEN + LM_HLEN + 16, sizeof(uint16_t));
		...
		visitingSrcIP = ntohl(visitingSrcIP);
		homeSrcIP = ntohl(homeSrcIP);
		if(LOGGING){
			os_WriteLog1("New src ip addr=%08x\n",visitingSrcIP);
			os_WriteLog1("Old src ip addr=%08x\n",homeSrcIP);
		}

		pre16 = (uint16_t) ((visitingSrcIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (visitingSrcIP & 0x0000ffff);
		if(LOGGING){
		...
		pre16 = (uint16_t) ((homeSrcIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (homeSrcIP & 0x0000ffff);
		...
		if (proto == IPPROTO_UDP) {
			memcpy(data + IP_HLEN + LM_HLEN + 6, &org_tp_check, sizeof(uint16_t));
		} // UDP calculates checksum

		else if (proto == IPPROTO_TCP) {
			memcpy(data + IP_HLEN + LM_HLEN + 16, &org_tp_check, sizeof(uint16_t));
		}
...
static int32_t lm_RemoveHeader(struct sk_buff *skb, uint32_t homeIP, uint32_t visitingIP, int proto)
{
	...
	memmove(skb->data + LM_HLEN, skb->data, ETH_HLEN + IP_HLEN);
	skb_pull(skb, LM_HLEN);
	data = skb->data + ETH_HLEN;
	...
	memcpy(data + 10, (void*)"\x0000", 2);
	memcpy(data + 16, (void*)&visitingIP, sizeof(visitingIP));
	temp = htons(CalculateIPChecksum(data, IP_HLEN));
	memcpy(data + 10, (void*)&temp, 2);
	
	if (proto != IPPROTO_ICMP && homeIP != visitingIP) {
...
//		
		...
		if(LOGGING){os_WriteLog1("Tp check 1s cmpl=%04x\n",org_tp_check);}
		homeIP = ntohl(homeIP);
		visitingIP = ntohl(visitingIP);
		if(LOGGING){
			os_WriteLog1("New dst ip addr=%08x\n",visitingIP);
			os_WriteLog1("Old dst ip addr=%08x\n",homeIP);
		}

		pre16 = (uint16_t) ((homeIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (homeIP & 0x0000ffff);
		if(LOGGING){
		...
		pre16 = (uint16_t) ((visitingIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (visitingIP & 0x0000ffff);
...
//Jaehyun created 2018/03/13
//Jaehee modified 2018/03/13
//Jaehee modified 2018/03/16
static int32_t lm_ChangeHeader(struct sk_buff *skb, uint32_t homeDstIP, uint32_t visitingDstIP, uint32_t homeSrcIP, uint32_t visitingSrcIP, int proto)
{
	uint8_t* data = NULL;
	uint16_t len = 0, temp = 0;
	...
	//char *pseudogram = NULL;
	//if(LOGGING){print_hex_dump(KERN_ALERT, "(pre)skb->data ", DUMP_PREFIX_OFFSET, 16, 1, skb->data, ntohs(*(uint16_t*)(skb->data + 2 + ETH_HLEN))+ETH_HLEN, 1);}
	if(LOGGING){os_WriteLog("Change header operation.\n");}

	if (homeDstIP != visitingDstIP || homeSrcIP != visitingSrcIP) {
		data = skb->data + ETH_HLEN;
	
		memcpy(data + 10, (void*)"\x0000", 2);
		memcpy(data + 16, (void*)&visitingDstIP, sizeof(visitingDstIP));
		temp = htons(CalculateIPChecksum(data, IP_HLEN));
		memcpy(data + 10, (void*)&temp, 2);
		if(LOGGING){os_WriteLog4("Revised dst IP=%u.%u.%u.%u\n",*((uint8_t*)&visitingDstIP + 0), *((uint8_t*)&visitingDstIP + 1), *((uint8_t*)&visitingDstIP + 2), *((uint8_t*)&visitingDstIP + 3));}
	}
	if (proto != IPPROTO_ICMP && (homeDstIP != visitingDstIP || homeSrcIP != visitingSrcIP)) {
...
//		
		...
		if (homeDstIP != visitingDstIP) {
					
			org_tp_check = ~ntohs(org_tp_check);
			if(LOGGING){os_WriteLog1("tp check 1s cmpl=%04x\n",org_tp_check);}
			
			sum = 0;
			homeDstIP = ntohl(homeDstIP);
			visitingDstIP = ntohl(visitingDstIP);
			if(LOGGING){
				os_WriteLog1("New dst ip addr=%08x\n",visitingDstIP);
				os_WriteLog1("Old dst ip addr=%08x\n",homeDstIP);
			}

			pre16 = (uint16_t) ((homeDstIP & 0xffff0000) >> 16);
			post16 = (uint16_t) (homeDstIP & 0x0000ffff);
			if(LOGGING){
				os_WriteLog1("Old dst ip preaddr=%04x\n",pre16);
				os_WriteLog1("Old dst ip postaddr=%04x\n",post16);}
			...
			pre16 = (uint16_t) ((visitingDstIP & 0xffff0000) >> 16);
			post16 = (uint16_t) (visitingDstIP & 0x0000ffff);
...
static uint8_t lm_GetSwitchNum(struct sk_buff* skb)
{
...
//Jaehee modified 2018/02/19
//Jaehee modified 2018/02/20
//Jaehee modified 2018/03/12
//Jaehee, Jaehyun modified 2018/03/13
static int32_t lm_CheckHeader(struct vport *vp, struct sk_buff *skb)
{
	uint8_t switchNum = 0;
	...
	uint8_t IHL = 0, tp_protocol = 0; //IHL = Protocol version number + IP Header Length
									...
									//lm header added, 44(sha1) -> 56(sha256)
	uint32_t srcIP = 0, dstIP = 0;
	uint16_t totalLen = 0, srcPort = 0, dstPort = 0;
	uint32_t homeIP = 0, newLocIP = 0, homeDstIP = 0, visitingDstIP = 0;
	...
	if (data[ETH_ALEN+0] == 0x24 && data[ETH_ALEN+1] == 0xdb && data[ETH_ALEN+2] == 0xed && data[ETH_ALEN+3] == 0xf0) {senderType = SENDERTYPE_UE;}
	...
	if (protocol == ETH_P_ARP && senderType == SENDERTYPE_UE) {
		lm_CheckNewUE(switchNum, protocol, data);
		...
		thisSWIP = SWITCHS_IP[switchNum-1];
		...
		if (senderType==SENDERTYPE_UE && !dstIPisSWIP) {
			if(LOGGING){
				...
				os_WriteLog8("New packet. Src IP=%u.%u.%u.%u, Dst IP=%u.%u.%u.%u\n", 
			...
			*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3));
			...
			if(LOGGING){
				os_WriteLog8("Check header. Src IP=%u.%u.%u.%u, Dst SWIP=%u.%u.%u.%u\n",*((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3), 
			*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3));
			}
		}
		...
		if (tp_protocol == IPPROTO_ICMP){
			if(LOGGING){
				os_WriteLog8("ICMP packet check header. Src IP=%u.%u.%u.%u, Dst IP=%u.%u.%u.%u\n",*((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3), 
				*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3));
				}
			...
			if ((IHL == 0x45 && (totalLen - IP_HLEN >= 18) && ((dstIP&0x00ffffff)!=(SWITCHS_IP[switchNum-1]&0x00ffffff)) && dstIP != 16777343 && dstIP != SWITCHS_IP[switchNum-1])||(senderType == SENDERTYPE_UE && (totalLen - IP_HLEN >= 18) && IHL == 0x45 && !dstIPisSWIP)) {
				...
				homeSrcIP = srcIP;
				if (srcIP != 16777343 && srcIP != SWITCHS_IP[switchNum-1] && lm_GetHomeIPFromSrcIP(switchNum, srcIP, 0, &hashed, &homeSrcIP)) {
					memcpy(ptrSrcIP,&homeSrcIP,sizeof(uint32_t));
					
					if(LOGGING){os_WriteLog9("ICMP, Switch num=%u, Src IP=%u.%u.%u.%u, Home Src IP=%u.%u.%u.%u\n", switchNum,
					*((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3),
					*((uint8_t*)&homeSrcIP + 0), *((uint8_t*)&homeSrcIP + 1), *((uint8_t*)&homeSrcIP + 2), *((uint8_t*)&homeSrcIP + 3));}
			
				}

				if (!lm_GetObjectFromIP(switchNum, dstIP, 0, &hashed, &newLocIP)) { // If it doesn't exist,
					dstPortByteArr[1] = 0;
					dstPortByteArr[0] = 0;
					// Send a request message to the upper layer
					ipc_SendMessage(switchNum, OPCODE_GET_HASH, dstIP, dstPortByteArr); 
					lm_SaveSKB(switchNum, dstIP, vp, skb);
					return DO_NOT_FORWARD;
					
				}
				if(LOGGING){os_WriteLog9("Switch num=%u, Dst IP=%u.%u.%u.%u, New Loc IP=%u.%u.%u.%u\n", switchNum,
					*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3),
				*((uint8_t*)&newLocIP + 0), *((uint8_t*)&newLocIP + 1), *((uint8_t*)&newLocIP + 2), *((uint8_t*)&newLocIP + 3));}
				
				
				if (newLocIP == 0) {
					return lm_AddHeader(skb, srcIP, hashed, 0, IPPROTO_ICMP, 0); //new IP is 0 and doInsertObjID is 0
				}

				if(LOGGING){os_WriteLog8("Locator es IP=%u.%u.%u.%u, this sw IP=%u.%u.%u.%u\n", 
				*((uint8_t*)&newLocIP + 0), *((uint8_t*)&newLocIP + 1), *((uint8_t*)&newLocIP + 2), *((uint8_t*)&newLocIP + 3), 
				*((uint8_t*)&thisSWIP + 0), *((uint8_t*)&thisSWIP + 1), *((uint8_t*)&thisSWIP + 2), *((uint8_t*)&thisSWIP + 3));}

				if (newLocIP == thisSWIP && lm_GetObjectFromHash(switchNum, hashed, &homeDstIP, &visitingDstIP)) {
					return lm_ChangeHeader(skb, homeDstIP, visitingDstIP, homeSrcIP, srcIP, IPPROTO_ICMP);
					
				}
				
				return lm_AddHeader(skb, srcIP, hashed, newLocIP, IPPROTO_ICMP, 1); //new IP is newLocIP(ESIP) and doInsertObjID is 1
		...
		// --------------------------------------------------------------------------------
			...
			if (IHL == 0x45) {
				...
				if (!lm_GetObjectFromHash(switchNum, hashed, &homeIP, &visitingDstIP)) { // If it doesn't exist,
					// Send a request message to the upper layer
				...
				*((uint8_t*)&visitingDstIP + 0), *((uint8_t*)&visitingDstIP + 1), *((uint8_t*)&visitingDstIP + 2), *((uint8_t*)&visitingDstIP + 3));}
				...
				return lm_RemoveHeader(skb, homeIP, visitingDstIP, IPPROTO_ICMP);
		...
		if (tp_protocol == IPPROTO_TCP || tp_protocol == IPPROTO_UDP) {
			if (IHL == 0x45) {
				...
				srcPort = ntohs(*(uint16_t*)(data + IP_HLEN + LM_HLEN));
				dstPort = ntohs(*(uint16_t*)(data + IP_HLEN + LM_HLEN + 2));
			}
		...
		// --------------------------------------------------------------------------------
			if(LOGGING){
				os_WriteLog8("IP packet check header. Src IP=%u.%u.%u.%u, Dst SWIP=%u.%u.%u.%u\n",*((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3), 
			*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3));
			}
		...
		if ((senderType == SENDERTYPE_UE && IHL == 0x45) || (senderType == SENDERTYPE_SW && IHL == 0x45 && !dstIPisSWIP))  {
			hashed = NULL;
			// Check entry in order to append an LM header
			/*if (!lm_GetObjectFromIP(switchNum, dstIP, 0, &hashed, &visitingIP)) { // If it doesn't exist,
				// Send a request message to the upper layer
			...
			homeSrcIP = srcIP;
			if (srcIP != 16777343 && srcIP != SWITCHS_IP[switchNum-1] && lm_GetHomeIPFromSrcIP(switchNum, srcIP, srcPort, &hashed, &homeSrcIP)) {
				memcpy(ptrSrcIP,&homeSrcIP,sizeof(uint32_t));
				if(LOGGING){os_WriteLog10("Switch num=%u, Src IP=%u.%u.%u.%u, src Port=%u, Home Src IP=%u.%u.%u.%u\n", switchNum,
				*((uint8_t*)&srcIP + 0), *((uint8_t*)&srcIP + 1), *((uint8_t*)&srcIP + 2), *((uint8_t*)&srcIP + 3), srcPort,
				*((uint8_t*)&homeSrcIP + 0), *((uint8_t*)&homeSrcIP + 1), *((uint8_t*)&homeSrcIP + 2), *((uint8_t*)&homeSrcIP + 3));}
			
			} 
			
			if (dstIP != 16777343 && dstIP != SWITCHS_IP[switchNum-1] && !lm_GetObjectFromIP(switchNum, dstIP, dstPort, &hashed, &newLocIP)) { // If it doesn't exist,
			...
			if(LOGGING){os_WriteLog10("Switch num=%u, Dst IP=%u.%u.%u.%u, dst Port=%u, New Loc IP=%u.%u.%u.%u\n", switchNum,
			...
			*((uint8_t*)&newLocIP + 0), *((uint8_t*)&newLocIP + 1), *((uint8_t*)&newLocIP + 2), *((uint8_t*)&newLocIP + 3));}
			...
			if (newLocIP == 0) {
				return lm_AddHeader(skb, srcIP, hashed, 0, tp_protocol, 0); //new IP is 0 and doInsertObjID is 0
			}
			...
			if (newLocIP == thisSWIP && lm_GetObjectFromHash(switchNum, hashed, &homeDstIP, &visitingDstIP)) {
				return lm_ChangeHeader(skb, homeDstIP, visitingDstIP, homeSrcIP, srcIP, tp_protocol);
			}
			
			return lm_AddHeader(skb, srcIP, hashed, newLocIP, tp_protocol, 1); //new IP is newLocIP(ESIP) and doInsertObjID is 1
		}
		...
		// --------------------------------------------------------------------------------
			...
			if(LOGGING){os_WriteLog10("Switch num=%u, Received IP=%u.%u.%u.%u, dst Port=%u, Host IP=%u.%u.%u.%u\n", switchNum,
				*((uint8_t*)&dstIP + 0), *((uint8_t*)&dstIP + 1), *((uint8_t*)&dstIP + 2), *((uint8_t*)&dstIP + 3), dstPort,
			*((uint8_t*)&visitingDstIP + 0), *((uint8_t*)&visitingDstIP + 1), *((uint8_t*)&visitingDstIP + 2), *((uint8_t*)&visitingDstIP + 3));
			os_WriteLog4("Home IP=%u.%u.%u.%u\n", *((uint8_t*)&homeIP + 0), *((uint8_t*)&homeIP + 1), *((uint8_t*)&homeIP + 2), *((uint8_t*)&homeIP + 3));}
			if (visitingDstIP == 0) {
				return DO_FORWARD;
			}
			...
			if (tp_protocol == IPPROTO_UDP) { 
				...
				*(uint16_t*)(data + IP_HLEN + LM_HLEN + 6) = (uint16_t)0;	
			
				uint16_t udp_check = CalculateUDPChecksum(srcIP, visitingDstIP, srcPort, dstPort, 
					data + IP_HLEN + LM_HLEN + UDP_HLEN, ntohs(*(uint16_t*)(data + IP_HLEN + LM_HLEN + 4)) - UDP_HLEN);
	
				memcpy(data + IP_HLEN + LM_HLEN + 6, (void*)&udp_check, sizeof(udp_check));
			...
			return lm_RemoveHeader(skb, homeIP, visitingDstIP, tp_protocol);
		}
	}
		       ...
		       struct ovs_key_ipv4_tunnel *tun_key)
{
	...
	// LOHan: Statistics Support
	/*uint8_t switchNum = lm_GetSwitchNum(skb);
	do_gettimeofday(&START_TIME);*/
	...
	/*if (switchNum == 7) {
		...
		if (randnum >= 8) {
			...
			if (lm_CheckHeader(vport, skb) == -1) return;
		}
	}
	else */if (lm_CheckHeader(vport, skb) == -1) {return;}
