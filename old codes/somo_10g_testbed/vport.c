...
static int32_t moe_AddHeader(struct sk_buff *skb, uint32_t newSrcIP, uint8_t* hashed, uint32_t esIP, int proto);
		...
		hash_for_each_possible(OBJ_TBL, current_entry, hlist_ip, destIP) {
			if (current_entry->switchNum == switchNum &&
				...
				*pSwitchIP = current_entry->switchIP;
				...
				if(LOGGING){os_WriteLog4("ESIP=%u.%u.%u.%u", *((uint8_t*)pSwitchIP + 0), *((uint8_t*)pSwitchIP + 1), *((uint8_t*)pSwitchIP + 2), *((uint8_t*)pSwitchIP + 3));}
				if(LOGGING){os_WriteLog("ObjID=\n");}
				if(LOGGING){
					for (i=0 ; i<(HASH_LEN/8) ; i++) {
						os_WriteLog8("%02x%02x%02x%02x%02x%02x%02x%02x\n", *((uint8_t*)*objHash+((i*8))),*((uint8_t*)*objHash+((i*8)+1)),*((uint8_t*)*objHash+((i*8)+2)),*((uint8_t*)*objHash+((i*8)+3)),*((uint8_t*)*objHash+((i*8)+4)),*((uint8_t*)*objHash+((i*8)+5)),*((uint8_t*)*objHash+((i*8)+6)),*((uint8_t*)*objHash+((i*8)+7)));
...
static int32_t moe_AddHeader(struct sk_buff *skb, uint32_t newSrcIP, uint8_t* hashed, uint32_t esIP, int proto)
{
	...
	if (proto != IPPROTO_ICMP) { // && newSrcIP != oldSrcIP) {
		...
		*org_tp_check = ~ntohs(*org_tp_check);
		...
		if(LOGGING){os_WriteLog1("new src ip addr=%08x\n",newSrcIP);}
		if(LOGGING){os_WriteLog1("old src ip addr=%08x\n",oldSrcIP);}

		pre16 = (uint16_t) ((newSrcIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (newSrcIP & 0x0000ffff);
		if(LOGGING){os_WriteLog1("new src ip preaddr=%04x\n",pre16);}
		if(LOGGING){os_WriteLog1("new src ip postaddr=%04x\n",post16);}
		...
		if(LOGGING){os_WriteLog1("old src ip preaddr=%04x\n",pre16);}
		if(LOGGING){os_WriteLog1("old src ip postaddr=%04x\n",post16);}
...
static int32_t moe_RemoveHeader(struct sk_buff *skb, uint32_t oldIP, uint32_t newIP, int proto)
{
	...
	if (proto != IPPROTO_ICMP) { // && oldIP != newIP) {
		...
		*org_tp_check = ~ntohs(*org_tp_check);
		...
		oldIP = ntohl(oldIP);
		newIP = ntohl(newIP);
		if(LOGGING){os_WriteLog1("new dst ip addr=%08x\n",newIP);}
		if(LOGGING){os_WriteLog1("old dst ip addr=%08x\n",oldIP);}

		pre16 = (uint16_t) ((oldIP & 0xffff0000) >> 16);
		post16 = (uint16_t) (oldIP & 0x0000ffff);
		if(LOGGING){os_WriteLog1("old dst ip preaddr=%04x\n",pre16);}
		if(LOGGING){os_WriteLog1("old dst ip postaddr=%04x\n",post16);}
		...
		if(LOGGING){os_WriteLog1("new dst ip preaddr=%04x\n",pre16);}
		if(LOGGING){os_WriteLog1("new dst ip postaddr=%04x\n",post16);}
	...
	if (protocol == ETH_P_ARP && senderType == SENDERTYPE_UE) {
		...
		if (tp_protocol == IPPROTO_ICMP) {
			...
			if (IHL == 0x45 && (totalLen - IP_HLEN >= 18) && ((dstIP&0x00ffffff)!=(SWITCHS_IP[switchNum-1]&0x00ffffff)) && dstIP != 16777343 && dstIP != SWITCHS_IP[switchNum-1]) {
				...
				*((uint8_t*)&newIP + 0), *((uint8_t*)&newIP + 1), *((uint8_t*)&newIP + 2), *((uint8_t*)&newIP + 3));}
				...
				return moe_AddHeader(skb, srcIP, hashed, newIP, IPPROTO_ICMP);
		...
		if (senderType == SENDERTYPE_UE && IHL == 0x45) {
			...
			*((uint8_t*)&newIP + 0), *((uint8_t*)&newIP + 1), *((uint8_t*)&newIP + 2), *((uint8_t*)&newIP + 3));}
			...
			return moe_AddHeader(skb, srcIP, hashed, newIP, tp_protocol);
		}
